...
|---|---|---|---|
| 1 | Запит до сервера | * Host voidlinux.org:443 was resolved<br>> GET / HTTP/1.1<br>< HTTP/1.1 200 OK<br>< Content-Type: text/html; charset=utf-8<br><!DOCTYPE html> | Формування HTTP-запиту та отримання відповіді від вебсервера.<br>При використанні HTTPS отримано повний доступ до HTML-вмісту вебсторінки, тоді як HTTP-запит отримав відповідь із перенаправленням на захищене HTTPS-з'єднання. |
| 2 | Визначення IP-адреси | Name: voidlinux.org  Type: AAAA  TTL: 642  Section: Answer   IPAddress: 2606:50c0:8000::153<br>Name: voidlinux.org  Type: A  TTL: 241  Section: Answer   IPAddress: 185.199.109.153| Визначення IP-адреси домену та параметрів DNS-записів у середовищі Windows PowerShell командою Resolve-DnsName. |
| 3 | Встановлення захищеного з'єднання | * schannel: disabled automatic use of client certificate<br>expires: Mon, 07 Sep 2026 11:46:23 GMT<br>< Server: GitHub.com<br> | Встановлення захищеного HTTPS-з'єднання та перевірка сертифіката на актуальність. |
| 4 | Проблеми із сертифікатами | * schannel: disabled automatic use of client certificate<br>* schannel: next InitializeSecurityContext failed: SEC_E_CERT_EXPIRED (0x80090328) - Получен сертификат с истекшим сроком действия<br>* schannel: SNI or certificate check failed: SEC_E_WRONG_PRINCIPAL (0x80090322) - Главное конечное имя неверно<br>* schannel: SEC_E_UNTRUSTED_ROOT (0x80090325) - Цепочка сертификатов выпущена центром сертификации, не имеющим доверия | Виявлення проблеми з TLS-сертифікатом. В усіх трьої випадках з'єднання не встановлювалось через різні проблеми з сертифікатами щодо цього домену. |
| 5 | Використання IP-адреси для з'єднання | * IPv6: (none)<br>* IPv4: 104.154.89.105<br>*   Trying 104.154.89.105:443... | Використання IP-адреси для встановлення з'єднання. Вибір порту (443 чи 80). |
| 6 | Канальний та фізичний зв'язки | відсутні | не вказано в output |
...
|---|---|
| * ALPN: curl offers http/1.1 | наявні лише при спроможності домена підтримати https |
| * ALPN: server accepted http/1.1 | наявні лише при спроможності домена підтримати https |
...
> зумовлено різними портами (443 та 80, https та http відповідно).
...
> У пункті А3 на початку зіткнувся з тим, що команда Resolve-DnsName <ДОМЕН> не розпізнавалась в середовищі cmd. Після кількох невдалих спроб та звернувшись за допомогою до Його Величності Google, дізнався що перевіряти треба у Windows PowerShell. Отримавши output, спочатку не міг зрозуміти чому наче для одного домена виводить 2 ip та 2 TTL. Перевіривши, дійшов висновку що це 2 способи ip-адресаціх в Інтернеті - IP-v4 та IP-v6 для А та АААА відповідно. У додатку до роботи йшлося що треба використовувати команду curl.exe (для Windows PowerShell), проте команди в блоці А були виконані у cmd, де просто curl -v працював нормально.

**D.2. Чому саме така кількість груп у частині B**

*На якій підставі ухвалено рішення. Що змусило б його змінити.*

> Спочатку груп було 3: "Запити на протокол підключення"; "Запити діагностики" та "Запити до доменів із проблемними сертифікатами". Рядків до них надано не було. На занятті у понеділок 14.09 зрозумів, що виконав завдання не вірно, через що довелося переписати класифікацію.

> Спирався на важливу умову подану до завдання: "Групи впорядковано від найближчої до користувача (№ 1) до найближчої до апаратного забезпечення. Зайві рядки вилучити, за потреби — додати.". Додатково зібравши інформацію про алгоритм передачі даних між сервером та користувачем, розробив відповідну класифікацію.
...
|---|---|---|
| 1 | B1 | які рядки слід віднести до категорії Встановлення захищеного з'єднання |
...
|---|---|---|---|
| 1 | B1 класифікація | порядок класифікацій в завданні B1 | окремий пункт для рядків з опрацюванням домену google |
| 2 | А5 - output команд з блоку А5 | опис завдання А5 | - |
...
> 14.09.26: переробив таблицю в завданні B1.

> що команди curl: я використовував саме curl -v (без .exe) - працював у середовищі cmd. У середовищі Windows PowerShell використовував лише команду Resolve-DnsName <ВАШ_ДОМЕН>.